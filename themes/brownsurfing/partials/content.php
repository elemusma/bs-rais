...
if($layout == 'Content + Image'){
    ...
    echo '<section class="position-relative ' . get_sub_field('classes') . '" style="padding:50px 0;' . get_sub_field('style') . '" id="' . get_sub_field('id') . '">';
...
echo do_shortcode('[button href="' . esc_url( $link_url ) . '" target="' . esc_attr( $link_target ) . '"]' . esc_html( $link_title ) . '[/button]');
// echo '<a class="position-relative d-flex align-items-center" href="' . esc_url( $link_url ) . '" target="' . esc_attr( $link_target ) . '">' . esc_html( $link_title ) . '</a>';
echo '</div>';
echo '</div>';
endif;
        ...
        if(have_rows('columns_repeater')): while(have_rows('columns_repeater')): the_row();
            echo '<div class="col-lg-4 col-md-6 ' . get_sub_field('column_classes') . '">';
                ...
                while(have_rows('accordions')): the_row();
                    ...
                    echo '<a class="button" style="cursor:pointer;color:var(--accent-primary);">' . get_sub_field('title') . ' ' . wp_get_attachment_image(219,'full','',['class'=>'','style'=>'']) . '</a>';
                    echo '<div class="accordion-content">';
                    echo '<div class="accordion-content-inner">';
                    echo get_sub_field('content');
                    echo '</div>';
                    echo '</div>';
                    echo '</div>';
